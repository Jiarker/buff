...
private:
    ...
    struct CURVE_FITTING_COST
    {
        ...
        template<typename T>
        bool operator()(
            const T *const awt,//模板参数,有3维
            T *residual) const    
        {
            //spd = a * sin(w * (t + t0)) + (2.090 - a)
            //residual[0] = T(_y) - 0.9*sin(awt[0]*(T(_x)+awt[1])) - (2.090 - 0.9);
            residual[0] = T(_y) + (awt[0]/awt[1])*cos(awt[1]*(T(_x)+awt[2])) - (2.090-awt[0])*T(_x) - (awt[0]/awt[1])*cos(awt[1]*awt[2]);
            return true;
        }
