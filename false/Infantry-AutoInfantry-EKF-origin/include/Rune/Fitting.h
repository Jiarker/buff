...
public:
    ...
    vector<AngleSpeedTime> fitting_data;
    vector<double> est_a;
    //double t_0 = 0.0; // 初相
...
//private:
    ...
    AngleSpeedTime correct_angle_speed_time;//预测后的角速度与时间
    SpdPredictor spdpredictor;//卡尔曼滤波器
    // bool spd_predictor_setting = false;//卡尔曼滤波开关
    // double last_angle = 0;//存储第一次角度与时间
...
public:
     ...
     *  @brief  数据作插值处理
     */
    void pushFittingData(AngleSpeedTime angle_speed_time);
...
private:
    ...
    {   //恰好在armor_new处发生角度突变
        if(armor_new.angle - armor_old.angle < -M_PI)//Ë³Ê±Õë
            ...
            angle_T -= 1;
        spdpredictor.initState(AngleSpeedTime((armor_new.angle + angle_T*2*M_PI + armor_old.angle)/2,
                                                (armor_new.angle + angle_T*2*M_PI - armor_old.angle)/(armor_new.gyro_pose.timestamp - armor_old.gyro_pose.timestamp),
                                                (armor_new.gyro_pose.timestamp + armor_old.gyro_pose.timestamp)/2
                                                ));
    };
     ...
     *  @return 返回预测后correct_angle_speed_time
     */
    ...
    double get_F(int n, int _N)
    {
        ...
        if (is_clockwise)
            ...
            {
                c += get_F_c(n, (fitting_data[i].speed - (2.090 - _a)), i, N);
                s += get_F_s(n, (fitting_data[i].speed - (2.090 - _a)), i, N);
            }
        else
            for (int i = 0; i < fitting_data.size(); i++)
            {
    ...
    double get_integral(double t_)
    {
        ...
        if (is_clockwise)
            for (int i = 0; i < fitting_data.size(); i++)
            {
                sum += sin((i * DT + t_) * _w) * (fitting_data[i].speed - (2.090 - _a)) / _a;
            }
        else
            for (int i = 0; i < fitting_data.size(); i++)
            {
    ...
    double get_variance(double t_)
    {
        ...
        if (is_clockwise)
            for (int i = 0; i < fitting_data.size(); i++)
            {
                sum += pow((sin((i * DT + t_) * _w) - (fitting_data[i].speed - (2.090 - _a)) / _a), 2);
            }
        else
            for (int i = 0; i < fitting_data.size(); i++)
            {
