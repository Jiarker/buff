...
typedef struct RuneParam
{
    ...
    float in_vane_width_offset;                 //  宽度偏移量
    ...
    float out_vane_width_offset;                 //  宽度偏移量
    ...
    //圆心
    //暂时不对最大距离进行判断
    float min_R_in_area_ratio;                   // R标志面积与内扇叶面积比
...
public:
    ...
    float short_side;       //  短边
    float long_side;        //  长边
    float ls_ratio;         //  长短边之比
    
    float contour_area;     //  轮廓面积
    float contour_rrect_ratio;//轮廓面积/外接矩形面积 
