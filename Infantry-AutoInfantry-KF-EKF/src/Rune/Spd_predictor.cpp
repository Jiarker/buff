...
void KFPredictor::initState(double angle, uint32_t time)
{
    angle_T = 0;
    cv::Mat statePost = (cv::Mat_<float>(StateNum, 1) << angle, 0);
...
double KFPredictor::predict(double angle, uint32_t time)
{
        ...
        //    <<std::endl;
    if(angle + 2*M_PI*angle_T - last_angle < -M_PI)//顺时针旋转引起角度突变
        angle_T += 1;
    else if(angle + 2*M_PI*angle_T - last_angle > M_PI)//逆时针旋转引起角度突变
        angle_T -= 1;
    ...
    cv::Mat measurement = (cv::Mat_<float>(MeasureNum, 1) << angle + angle_T*2*M_PI);
