...
{
    // 待激活目标，角点数少，内外轮廓面积比较大
    if (//abs(vane.hull_num - armor_2.vane.hull_num) > 10 && 
        vane.hull_num > armor_2.vane.hull_num)
        return true;
    else
    {
        // float ratio1 = contourArea(contour) / contourArea(vane.contour);
        // float ratio2 = contourArea(armor_2.contour) / contourArea(armor_2.vane.contour);
