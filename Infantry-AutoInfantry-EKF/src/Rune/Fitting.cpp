...
bool FitTool::runNormalRune(RuneArmor armor_1, vector<cv::Point2f> &nextPosition, ArmorState armor_state)
{
    ...
    case ArmorState::FIRST:
        ...
        //????????????
        //spdpredictor.initState(AngleSpeedTime(armor_1.angle, armor_1.gyro_pose.timestamp));
    ...
    case ArmorState::SHOOT:
        ...
        if(armor_buffer.size() == 2)//??????????
            Kalman_init(armor_1, armor_buffer[armor_buffer.size()-1]);
        //Kalman_predict(armor_1, armor_buffer[armor_buffer.size() - 1]);
        if (armor_buffer.size() >= DN + 1)
        {
            pushFittingData(correct_angle_speed_time);
...
bool FitTool::processDataState(RuneArmor armor_1, ArmorState armor_state)
{
    ...
    case ArmorState::SHOOT:
        if (armor_1.gyro_pose.timestamp <= 10 || armor_buffer.empty())
            return false;
        if(armor_buffer.size() == 1)
            Kalman_init(armor_1, armor_buffer[0]);
        else
        {
            Kalman_predict(armor_1, armor_buffer[armor_buffer.size() - 1]);
            armor_buffer.push_back(armor_1);
...
void FitTool::pushFittingData(AngleSpeedTime new_data)
{
    ...
    AngleSpeedTime flag_data = fitting_data[fitting_data.size() - 1];//末尾数据
    if ((double)new_data.time - (double)flag_data.time - DT * 1000.0 < 0)//未满足线性插值条件
    {
        return;
    }
    ...
    for (int i = 0; i < (int)n; i++)//线性插值
    {
        ...
        double temp_speed = new_data.speed * (temp_T / (temp_T + delta)) + flag_data.speed * (delta / (temp_T + delta));
        uint32_t temp_time = flag_data.time + (uint32_t)temp_T;
        AngleSpeedTime temp = AngleSpeedTime(temp_speed, temp_time);
        //AngleSpeedTime temp = AngleSpeedTime(new_data.angle_speed * (temp_T / (temp_T + delta)) + flag_data.angle_speed * (delta / (temp_T + delta)), flag_data.time + (uint32_t)temp_T);  
...
double FitTool::calAngleSpeed(RuneArmor armor_1, RuneArmor armor_2)
{
    ...
    if (armor_1.angle < -CV_PI / 2.0 && armor_2.angle > CV_PI / 2.0)
        ...
        angle_diff = angle_diff - CV_PI * 2.0;
    return angle_diff / time_diff; // 角速度
}
...
{
    if(armor_new.angle - armor_old.angle < -M_PI)//顺时针
        angle_T += 1;
    else if(armor_new.angle - armor_old.angle > M_PI)//逆时针
        angle_T -= 1;
    correct_angle_speed_time = spdpredictor.predict(AngleSpeedTime(armor_new.angle + 2*M_PI*angle_T,0, armor_new.gyro_pose.timestamp));
}
